            ...
            {SidebarContent.map((item, index) => {
              return (
                <li className={selected === index  ? "mb-4 group active" : "mb-4" } key={index} onClick={() => setSelected(index)}>
                  <Link to="#" className={`flex w-full rounded-md items-center space-x-2 text-gray-200  hover:bg-[#080e24] hover:text-white group-[.active]:bg-gray-700 group-[.active]:text-white ${colapse ? 'text-xl': 'text-3xl'}`}>
                    { colapse ? (
                      <>
                        <item.icon />
